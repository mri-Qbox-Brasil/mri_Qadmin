...
export const InventoryItem: React.FC<InventoryItemProps> = ({
  ...
  return (
    <div
      ...
      className={cn(
        "relative aspect-[0.9/1] rounded bg-card border border-border flex flex-col items-center justify-between p-1 overflow-hidden group hover:border-primary/40 transition-all cursor-help",
        className
      )}
    >
      {/* Top Bar: Weight & Count */}
      <div className="w-full flex justify-between items-start px-1 pt-0.5 z-10">
      ...
      <div className="flex-1 flex items-center justify-center w-full p-2 relative">
        <img
          src={finalIcon}
          alt={label}
          className="max-w-full max-h-full object-contain filter drop-shadow-[0_0_8px_rgba(0,0,0,0.5)] group-hover:scale-110 transition-transform duration-300"
          onError={(e) => {
            if (name && !(e.target as HTMLImageElement).src.endsWith('unknown.png')) {
              (e.target as HTMLImageElement).src = 'nui://ox_inventory/web/images/unknown.png'
            }
          }}
        />
      ...
      {/* Bottom Label */}
      <div className="w-full bg-muted/80 py-1 px-1.5 border-t border-border mt-auto">
