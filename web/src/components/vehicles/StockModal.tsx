...
export default function StockModal({ vehicle, isOpen, onClose, onConfirm }: StockModalProps) {
    ...
    // Reset stock when modal opens with a new vehicle (State update during render pattern)
    const [prevVehicleId, setPrevVehicleId] = useState(vehicle?.id || vehicle?.model)
    if (isOpen && (vehicle?.id || vehicle?.model) !== prevVehicleId) {
        setPrevVehicleId(vehicle?.id || vehicle?.model)
        setStock(vehicle?.stock || 0)
    }
