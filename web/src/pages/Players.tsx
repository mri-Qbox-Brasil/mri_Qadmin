...
export default function Players() {
    ...
    const [filterGroup] = useState<'all' | 'online' | 'offline'>('all')
    ...
    const filteredPlayers = useMemo(() => {
        return players.filter((p: Player) => {
            ...
            const matchesGroup = filterGroup === 'all' || (filterGroup === 'online' && p.online) || (filterGroup === 'offline' && !p.online)
                ...
                {viewMode === 'list' && (
                    <div className="h-full flex">
                        <div className="w-96 flex flex-col border-r border-border bg-card/10 h-full relative">
                            <div className="pt-4 p-2 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{t('all_players')} ({pagination.total})</div>
                ...
                {selectedPlayer && (
                    <div className={cn(
                                ...
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <div className="flex items-center gap-2 pt-1">
                                        ...
                                        <span className={cn(
                                            ...
                                            {((selectedPlayer.health || 0) <= 101 || selectedPlayer.metadata?.isdead) ? t('vitals_status_dead') : t('vitals_status_alive')}
                                        </span>
                        ...
                        <div className="flex-1 overflow-y-auto p-2 space-y-8">
                            {selectedPlayer.online && (
                                <section>
                                    <MriSectionHeader icon={Heart} title={t('vitals_section_title')} />
                                    <MriPlayerVitals
                                        vitals={{
                            ...
                            <section>
                                ...
                                <div className="grid grid-cols-6 gap-3">
                                    <MriGridActionButton icon={Check} label={`${t('verify')} `} onClick={() => sendAction({ event: 'mri_Qadmin:server:verifyPlayer', type: 'server', perms: 'qadmin.action.verify_player' })} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={Heart} label={`${t('revive')} `} onClick={() => sendAction({ event: 'mri_Qadmin:server:Revive', type: 'server', perms: 'qadmin.action.revive' })} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={Skull} label={`${t('kill')} `} onClick={() => sendAction({ event: 'mri_Qadmin:server:KillPlayer', type: 'server', perms: 'qadmin.action.kill_player' })} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={User} label={`${t('clothing')} `} onClick={() => sendAction({ event: 'mri_Qadmin:server:ClothingMenu', type: 'server', perms: 'qadmin.action.clothing_menu' })} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={Lock} label={`${t('toggle_cuffs')} `} onClick={() => sendAction({ event: 'mri_Qadmin:server:CuffPlayer', type: 'server', perms: 'qadmin.action.toggle_cuffs' })} disabled={!selectedPlayer.online} />
                                </div>
                            </section>

                            <section>
                                <MriSectionHeader icon={Eye} title={t('moderation_section')} />
                                <div className="grid grid-cols-6 gap-3">
                                    <MriGridActionButton icon={Eye} label={`${t('spectate')} `} onClick={() => sendAction({ event: 'mri_Qadmin:server:SpectateTarget', type: 'server', perms: 'qadmin.action.spectate_player' })} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={Lock} label={`${t('toggle_freeze')} `} onClick={() => sendAction({ event: 'mri_Qadmin:server:FreezePlayer', type: 'server', perms: 'qadmin.action.freeze_player' })} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={AlertTriangle} label={`${t('warn')} `} variant="warning" onClick={() => setShowWarnModal(true)} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={LogOut} label={`${t('kick')} `} onClick={() => setShowKickModal(true)} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={Ban} label={`${t('ban')} `} variant="destructive" onClick={() => setShowBanModal(true)} />
                                </div>
                            </section>

                            <section>
                                <MriSectionHeader icon={Crosshair} title={t('teleportation_section')} />
                                <div className="grid grid-cols-6 gap-3">
                                    <MriGridActionButton icon={Crosshair} label={`${t('go_to')} `} onClick={() => sendAction({ event: 'mri_Qadmin:server:TeleportToPlayer', type: 'server', perms: 'qadmin.action.teleport_to_player' })} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={Download} label={`${t('bring')} `} onClick={() => sendAction({ event: 'mri_Qadmin:server:BringPlayer', type: 'server', perms: 'qadmin.action.bring_player' })} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={Undo} label={`${t('send_back')} `} onClick={() => sendAction({ event: 'mri_Qadmin:server:SendPlayerBack', type: 'server', perms: 'qadmin.action.teleport_back' })} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={Navigation} label={`${t('set_bucket')} `} onClick={() => setShowBucketModal(true)} disabled={!selectedPlayer.online} />
                                    <MriGridActionButton icon={MapIcon} label={`${t('track_player') || 'Map'} `} onClick={() => setShowMapModal(true)} disabled={!selectedPlayer.online} />
                                </div>
                            </section>

                            <section>
                                <MriSectionHeader icon={Wallet} title={t('economy_groups')} />
                            ...
                            <section>
                                <MriSectionHeader icon={Car} title={t('player_vehicles')} />
                            ...
                            <section>
                                <MriSectionHeader icon={ExternalLink} title={t('inventory_management')} />
                                <div className="grid grid-cols-4 gap-3">
                            ...
                            {!selectedPlayer.online && (
                                <section>
                                    <MriSectionHeader icon={Skull} title={t('offline_actions')} />
                                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                                        <MriGridActionButton icon={Trash2} label={`${t('delete_character')} `} variant="destructive" onClick={() => sendAction({ event: 'mri_Qadmin:server:delete_cid', type: 'server', perms: 'qadmin.action.delete_character' }, { cid: { value: selectedPlayer.cid } })} />
                                        <MriGridActionButton icon={Ban} label={`${t('unban')} `} onClick={() => sendAction({ event: 'mri_Qadmin:server:unban_cid', type: 'server', perms: 'qadmin.action.unban_player' }, { cid: { value: selectedPlayer.cid } })} />
                                    </div>
                                </section>
                            )}
                            ...
                            <div className="p-4 border-t border-border bg-card/20 text-xs text-mono text-muted-foreground mt-auto">
                                ...
                                <div className="grid grid-cols-2 gap-x-8 gap-y-2">
                                    ...
                                    <div className="flex justify-between items-center text-xs font-mono">
                                        ...
                                        <span className="text-foreground">{Math.floor(selectedPlayer.health ?? 0)}%</span>
                                    </div>
                                    <div className="w-full bg-sidebar-foreground/10 rounded-full h-1 no-scrollbar overflow-hidden">
                                        <div
                                            className="bg-green-500 h-full transition-all duration-500"
                                            style={{ width: `${Math.floor(selectedPlayer.health ?? 0)}% ` }}
                                        />
                                    </div>
                                    <div className="flex justify-between border-b border-border/50 pb-1">
                                        <span>{t('license')}</span>
