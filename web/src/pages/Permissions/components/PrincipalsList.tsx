...
export default function PrincipalsList({
    ...
    // Correctly report count to parent when principals change
    useEffect(() => {
    ...
    const fetchPrincipals = useCallback(async () => {
        ...
        try {
            if (isEnvBrowser()) {
                setTimeout(() => {
                    setPrincipals(prev => (prev.length === 0 ? MOCK_PRINCIPALS : prev));
                    setLoading(false);
                }, 500);
                return;
            }
            ...
            // onCountChange?.(unique); handled in useEffect
    ...
    useEffect(() => {
        fetchPrincipals();
    }, [refreshTrigger, fetchPrincipals]);
                ...
                <div className="h-full">
                    {loading && principals.length === 0 ? (
                        ...
                        (() => {
                            ...
                            // Group by child
                            const groupedEntries: { child: string; items: Principal[] }[] = Object.entries(grouped).map(([child, items]) => {
                            ...
                            return (
                                <Virtuoso
                                    style={{ height: '100%' }}
                                    data={groupedEntries}
                                    itemContent={(_: number, { child, items }: { child: string; items: Principal[] }) => (
                                        <div className="pb-3">
                                            <PrincipalGroup
                                                child={child}
                                                items={items}
