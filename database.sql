...
-- Whitelist durável dos nós registrados por plugins. A lista em memória do Qadmin é
-- reconstruída a cada start e só recebe nós de plugin quando o plugin se registra. Sem esta
-- tabela, um restart do Qadmin (ou um plugin parado) faria um save de grupo apagar do banco
-- permissões de plugin que estavam salvas e funcionando.
CREATE TABLE IF NOT EXISTS mri_qadmin_known_permissions (
    permission VARCHAR(255) PRIMARY KEY,
    source VARCHAR(100) NOT NULL DEFAULT 'plugin',
    createdAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
