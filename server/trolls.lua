...
RegisterNetEvent('mri_Qadmin:server:FreezePlayer', function(_, selectedData)
    ...
    if not frozen then
        ...
        AddLog(src, 'mri_Qadmin', 'players', 'warn', ('Congelar: %s foi congelado'):format(playerName), { target = target, citizenid = Player.PlayerData.citizenid })
    else
        frozen = false
        FreezeEntityPosition(ped, false)
        QBCore.Functions.Notify(src,
            locale("notifications.deFrozen", playerName .. " | " .. Player.PlayerData.citizenid), 'Success', 7500)
        AddLog(src, 'mri_Qadmin', 'players', 'info', ('Descongelar: %s foi descongelado'):format(playerName), { target = target, citizenid = Player.PlayerData.citizenid })
    end
...
RegisterNetEvent('mri_Qadmin:server:DrunkPlayer', function(_, selectedData)
    ...
    AddLog(src, 'mri_Qadmin', 'players', 'warn', ('Bêbado: efeito de embriaguez aplicado em %s'):format(playerName), { target = target, citizenid = Player.PlayerData.citizenid })
end)
