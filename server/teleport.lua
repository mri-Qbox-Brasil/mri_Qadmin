...
RegisterNetEvent('mri_Qadmin:server:BringPlayer', function(data, selectedData)
    ...
    print(('[mri_Qadmin] DEBUG BringPlayer: Admin Coords (Src: %s): %s'):format(src, adminCoords))
    ...
    if adminCoords.x == 0 and adminCoords.y == 0 and adminCoords.z == 0 then
         ...
         if pAdmin and pAdmin.PlayerData.position then
             print('[mri_Qadmin] Admin Entity Coords Invalid (0,0,0). Using QBCore Cached for Admin.')
             adminCoords = pAdmin.PlayerData.position
         else
             print('[mri_Qadmin] CRITICAL: Cannot determine Admin position. Aborting Bring to prevent sending player to void.')
    ...
    if rawCoords.x == 0 and rawCoords.y == 0 and rawCoords.z == 0 then
        ...
        if tPlayer then
            print(('[mri_Qadmin] BringPlayer: Target Entity Coords invalid (0,0,0). Using QBCore cached coords for ID %s'):format(targetId))
            rawCoords = tPlayer.PlayerData.position
        else
            print(('[mri_Qadmin] BringPlayer WARNING: Could not find QBCore player for ID %s to fix coords'):format(targetId))
        end
    else
        print(('[mri_Qadmin] BringPlayer: Saving Previous ID %s: %s'):format(targetId, rawCoords))
    end
    ...
    -- Execute Teleport
    print(('[mri_Qadmin] Teleporting ID %s to %s'):format(targetId, adminCoords))
...
RegisterNetEvent('mri_Qadmin:server:SendPlayerBack', function(data, selectedData)
    ...
    if lastPos then
        local targetPed = GetPlayerPed(targetId)
        print(('[mri_Qadmin] SendPlayerBack: Sending ID %s to %s'):format(targetId, lastPos))
        ...
        print(('[mri_Qadmin] SendPlayerBack: No saved position for ID %s'):format(targetId))
