-- ─── Disponibilidade ──────────────────────────────────────────────────────────
-- O mri_Qbox pode estar rodando numa versao sem os exports de VIP (getRanks etc).
-- Nesse caso a pagina VIP deve ficar oculta na NUI, entao a checagem valida o
-- export de fato (pcall), nao so o estado do resource. Resultado e cacheado e
-- invalidado quando o mri_Qbox (re)inicia.
local vipAvailable = nil

AddEventHandler('onResourceStart', function(res)
    if res == 'mri_Qbox' then vipAvailable = nil end
end)

function IsVipAvailable()
    if GetResourceState('mri_Qbox') ~= 'started' then
        vipAvailable = nil
        return false
    end
    if vipAvailable == nil then
        vipAvailable = pcall(function()
            return exports['mri_Qbox']:getRanks()
        end)
        if not vipAvailable then
            Debug('debug', ('[mri_Qadmin] VIP: export getRanks indisponivel no mri_Qbox — pagina VIP oculta'))
        end
    end
    return vipAvailable
end
...
lib.callback.register('mri_Qadmin:vip:getRanks', function(source)
    ...
    if not IsVipAvailable() then return {} end
...
lib.callback.register('mri_Qadmin:vip:createRank', function(source, data)
    ...
    if not IsVipAvailable() then return { status = 'error' } end
