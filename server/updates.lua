    ...
    PerformHttpRequest(RELEASES_URL, function(code, body, _headers)
        if code == 200 and body and body ~= '' then
            ...
            if ok and data and data.tag_name then
                ...
                Debug(('[mri_Qadmin] Update check: current=%s latest=%s available=%s')
                    :format(current, latest, tostring(updateInfo.updateAvailable)))
            else
                Debug('[mri_Qadmin] Update check: resposta invalida da API do GitHub')
            end
        else
            Debug(('[mri_Qadmin] Update check falhou (HTTP %s)'):format(tostring(code)))
        end
