local function GetItemsList()
    ...
    for item, data in pairs(rawItems) do
        ...
        if not itemLabel then
            Debug(('[mri_Qadmin] ^3AVISO: Item "%s" está mal formatado ou sem Label!^7'):format(tostring(itemName)))
            Debug(('[mri_Qadmin] Dados brutos do item "%s": %s'):format(tostring(itemName), json.encode(data)))
    ...
    if #Items == 0 then
        Debug('^1[mri_Qadmin] ERRO CRÍTICO: Nenhum item foi encontrado no Shared.Items ou ox_inventory!^7')
    else
        Debug(('[mri_Qadmin] Sucesso: %d itens carregados para a interface.'):format(#Items))
    end
