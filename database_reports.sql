-- MRI Qadmin Report System Database Schema
-- Run this SQL to set up the required tables for the Tickets tab
...
CREATE TABLE IF NOT EXISTS `mri_report_messages` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    ...
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (`report_id`) REFERENCES `mri_reports`(`id`) ON DELETE CASCADE,
    INDEX `idx_report_id` (`report_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Admin notes for specific reports (internal, player cannot see)
CREATE TABLE IF NOT EXISTS `mri_report_notes` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `report_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `mri_player_notes` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `player_id` VARCHAR(60) NOT NULL,
    `admin_id` VARCHAR(60) NOT NULL,
    `admin_name` VARCHAR(64) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `mri_player_identifiers` (
