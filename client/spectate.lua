...
RegisterNetEvent('mri_Qadmin:requestSpectate', function(target, _)
    -- Only capture the return position on the first activation, so re-triggering
    -- (e.g. switching target) doesn't overwrite it with the spectate location.
    if not spectateInfo.toggled then
